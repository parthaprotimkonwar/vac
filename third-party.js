function callAPI() {
  //document.getElementById("message").innerText = "status";
  const currentDate = getTodaysDateForUrl();
  const params = new URLSearchParams(document.location.search);
  
  const SPR_PINCODE = 785601;
  const GOLAGHAT_ID = 53;
  const pincode = params.get("pincode");
  const districtId = params.get("districtId");
  console.info(pincode); //show C
  console.info(districtId);
  //const url = "https://cdn-api.co-vin.in/api/v2/appointment/sessions/public/calendarByDistrict?district_id=53&date=11-05-2021"
                 //https://cdn-api.co-vin.in/api/v2/appointment/sessions/public/calendarByDistrict?district_id=53&date=11-05-2021
  var url = "https://cdn-api.co-vin.in/api/v2/appointment/sessions/public/";
  if( pincode != null && pincode != "") {
    url += "calendarByPin?pincode=" + pincode; 
  } else if(districtId != null && districtId != "") {
    document.getElementById("districtId").innerText = districtId;
    url += "calendarByDistrict?district_id=" + districtId;
  } else {
    //default to golaghat
    document.getElementById("districtId").innerText = 53;
    url += "calendarByDistrict?district_id=53";
  }
  url += "&date=" + currentDate;
  
  console.log(url);
  $.getJSON( url, function( data ) {

    //document.getElementById("message").innerText = data;
    console.log(data.centers);

    for(const center = 0; center <= data.centers.length; center++) {
      const sessions = data.centers[center].sessions;
      const name = data.centers[center].name;
      for(const session = 0; session <= sessions.length; session++) {
        const facility = sessions[session];
        if(facility.available_capacity > 0 || facility.available_capacity_dose1 >0 ||
          facility.available_capacity_dose2 >0) {
          document.getElementById("message").style.color = 'green';
          document.getElementById("message").innerText = "VACCINE AVAILABLE in " + name;
          console.log("AVAIL")
          beep();
          return;
        } else {
          document.getElementById("message").style.color = 'red';
          document.getElementById("message").innerText = "NOT AVAILABLE";
          console.log("NOT AVAIL")
          return;
        }
      }
    }
  });
}

function dateInIST() {
  const currentTime = new Date();
  const currentOffset = currentTime.getTimezoneOffset();
  
  const ISTOffset = 330;   // IST offset UTC +5:30 
  const ISTTime = new Date(currentTime.getTime() + (ISTOffset + currentOffset)*60000);
  return ISTTime;
}

function getTodaysDateForUrl() {
  const d = dateInIST();

  d.setMonth( d.getMonth() + 1 );
  const currentDate = d.getDate() < 10 ? '0' + d.getDate() : d.getDate();
  const currentMonth = d.getMonth() < 10 ? '0' + d.getMonth() : d.getMonth();
  const currentYear = d.getFullYear();
  return currentDate + "-" + currentMonth + "-" + currentYear; 
}

 function beep() {
  var snd = new Audio("data:audio/wav;base64,");  
  //snd.muted = true;
  var intervalId = window.setInterval(function(){
    /// call your function here
    snd.play();
    
  }, 1000);
} 

$(document).ready(function () {
  callAPI();
  //your code here
  var intervalId = window.setInterval(function(){
    /// call your function here
    callAPI();
  }, 10000);
  
});

